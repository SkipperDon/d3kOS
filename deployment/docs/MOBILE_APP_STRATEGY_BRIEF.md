...
## WHAT d3kOS IS — PRODUCT IDENTITY

**d3kOS — AI First Mate.**

d3kOS is a companion system to chartplotters and ActiveCaptain. It does not
duplicate navigation or community features. It adds the AI intelligence layer
that existing tools do not provide: boat health, engine monitoring, system
management, and AI analysis.

- **Chartplotter** → handles navigation, charts, waypoints
- **ActiveCaptain** → handles marina reviews, anchorages, community data
- **d3kOS AI First Mate** → handles boat systems intelligence, engine health,
  Pi management, AI analysis, and proactive monitoring

The three sit side by side on the helm and on the phone. d3kOS makes the
boat smarter — it does not replace the tools that already work.
...
### Decision 7 — Payments via Stripe, initiated from inside the PWA

User taps "Upgrade" or "Fix My Pi" inside the PWA. The PWA opens the
atmyboat.com payment page in an embedded browser view. Stripe payment
completes on atmyboat.com. PWA detects the tier change and unlocks features.
The user never leaves the PWA experience.

All payments processed by Stripe on atmyboat.com — 2.9% + 30c per transaction.
No App Store involvement. No 30% cut. Feature flags on AtMyBoat.com control
what each tier can access — no app code changes needed to add/remove features.
...
### OTA Upgrade from Phone (T1, T2, T3 only)

**T0 cannot use OTA — no mobile app, no remote connection. T0 upgrades require direct Pi access.**

For T1, T2, T3:
1. App shows new version available with release notes
2. User taps "Upgrade Now"
3. Command written to AtMyBoat.com command queue
...
Stored in central database. Downloadable from app. Available at T2 and T3 only.
...
d3kOS AI First Mate and ActiveCaptain are used side by side on the same phone.
They complement each other — d3kOS adds the AI intelligence layer ActiveCaptain
does not provide.
...
- d3kOS is the AI First Mate — companion to chartplotters and ActiveCaptain, not a duplicate of them
