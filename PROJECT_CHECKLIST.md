...
| 10 | Fish detection — Gemini Vision species ID | Claude | `[ ]` **Completion path confirmed 2026-03-21:** Add `/detect/identify` to fish_detector.py — sends captured JPEG to Gemini Vision API (key already on Pi). Returns Ontario-relevant species name. ONNX 483-species model is wrong dataset (Australian fish) and must NOT be used for species display. See `deployment/docs/FISH_DETECTION_ARCHITECTURE.md`. Awaiting authorization to build. |
