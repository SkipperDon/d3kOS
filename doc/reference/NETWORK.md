...
d3kOS creates a dual-interface network configuration:
...
**Connection File**: `/etc/NetworkManager/system-connections/d3kOS-AP`
...
**Auto-Detection Script** (`/opt/d3kos/scripts/network-check.sh`):
...
- Signal K Admin: `http://d3kos.local:3000/@signalk/server-admin-ui`
- Camera (Tier 2+): `rtsp://d3kos.local:554/camera`
...
**Hostname Resolution**:
- `d3kos.local` resolves to 10.42.0.1 on WiFi network
...
<service-group>
  <name replace-wildcards="yes">d3kOS Web Interface</name>
  <service>
...
<!DOCTYPE service-group SYSTEM "avahi-service.dtd">
<service-group>
  <name replace-wildcards="yes">d3kOS Signal K</name>
  <service>
...
<!DOCTYPE service-group SYSTEM "avahi-service.dtd">
<service-group>
  <name replace-wildcards="yes">d3kOS Node-RED</name>
  <service>
...
# Resolve hostname
avahi-resolve -n d3kos.local

# Check if hostname is published
avahi-resolve-host-name d3kos.local
```
...
```bash
# Using nmcli
sudo nmcli connection modify d3kOS-AP wifi-sec.psk "new_password_here"
sudo nmcli connection up d3kOS-AP

# Manually edit
sudo nano /etc/NetworkManager/system-connections/d3kOS-AP
# Change 'psk=' line
...
sudo nmcli connection modify d3kOS-AP wifi.hidden yes
sudo nmcli connection up d3kOS-AP
```
...
- Navigate to: `http://d3kos.local:3000/@signalk/server-admin-ui`
...
- Not typically needed for d3kOS
...
**Solutions**:
1. Verify password: `sudo nmcli connection show d3kOS-AP | grep psk`
2. Reset to default: `sudo nmcli connection modify d3kOS-AP wifi-sec.psk d3kos-2026`
3. Restart AP: `sudo nmcli connection up d3kOS-AP`
...
**Problem**: Can't access dashboard at d3kos.local

**Solutions**:
1. Try IP address instead: `http://10.42.0.1:1880/dashboard`
2. Check mDNS: `avahi-resolve -n d3kos.local`
...
**Problem**: d3kos.local not resolving
...
sudo nmcli connection modify d3kOS-AP wifi.channel 11
sudo nmcli connection up d3kOS-AP
```
...
# Restart AP
nmcli connection down d3kOS-AP && nmcli connection up d3kOS-AP

# Change SSID
nmcli connection modify d3kOS-AP wifi.ssid "New-SSID"

# Change password
nmcli connection modify d3kOS-AP wifi-sec.psk "new_password"

# Change channel
nmcli connection modify d3kOS-AP wifi.channel 11
```
...
| File | Purpose |
|------|---------|
| `/etc/NetworkManager/system-connections/d3kOS-AP` | WiFi AP configuration |
...
BACKUP_DIR="/opt/d3kos/backups/network_$(date +%Y%m%d_%H%M%S)"
mkdir -p $BACKUP_DIR
...
# WiFi AP
sudo nmcli connection modify d3kOS-AP wifi.ssid "d3kOS"
sudo nmcli connection modify d3kOS-AP wifi-sec.psk "d3kos-2026"
sudo nmcli connection modify d3kOS-AP ipv4.addresses "10.42.0.1/24"
