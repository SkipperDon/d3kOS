#!/usr/bin/env python3
"""
d3kOS Hybrid Voice Assistant v2 - Phase 2 Implementation
...
WAKE_WORDS_FILE = "/opt/d3kos/config/sphinx/wake-words.kws"
...
class HybridVoiceAssistant:
    ...
    def check_dependencies(self):
        ...
        checks = [
            ...
            (os.path.exists(WAKE_WORDS_FILE), f"Wake words file: {WAKE_WORDS_FILE}")
        ]
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                if "Answer:" in output:
                    ...
                    answer = lines[0].strip() if lines else answer_section
                    return answer
                else:
    ...
    def detect_wake_word_once(self):
        """Detect which wake word was spoken using pocketsphinx"""
        try:
            # Run pocketsphinx with keyword file
            ...
            for line in process.stderr:
                ...
                # Check which wake word was detected
    ...
    def run_service(self):
        print("="*60, flush=True)
        print("d3kOS Hybrid Voice Assistant v2 - Service Mode", flush=True)
        ...
        try:
            while True:
                ...
                if wake_word:
                    ...
                    print(f"\n{'='*60}", flush=True)
                    print(f"✓ Wake word detected: {wake_word.upper()} (AI mode: {config['ai']})", flush=True)
                    print("="*60, flush=True)
                    
                    # Speak acknowledgment
                    self.speak(config['response'])
                    
                    # Wait for microphone to fully release
                    time.sleep(0.5)
